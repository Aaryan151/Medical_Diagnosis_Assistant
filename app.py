...
st.set_page_config(
    page_title="Intelligent Medical Diagnosis Assistant",
    page_icon="🏥",
    layout="wide",
...
.disc {
    ...
    border-radius: 10px;
    padding: 12px 16px;
    font-size: 11.5px; color: #92400e; color: #a16207;
...
# ══════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        res  = models["heart_disease_model"].predict(f)[0]
        prob = models["heart_disease_model"].predict_proba(f)[0]
...
# ══════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        res  = models["diabetes_model"].predict(f)[0]
        prob = models["diabetes_model"].predict_proba(f)[0]
        conf = prob[1] if res == 1 else prob[0]
        show_result(res == 1, conf,
...
# ══════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        res  = models["lung_cancer_model"].predict(f)[0]
        prob = models["lung_cancer_model"].predict_proba(f)[0]
        conf = prob[1] if res == 1 else prob[0]
        show_result(res == 1, conf,
            ...
            "<strong>Recommendation:</strong> No lung cancer risk detected. Avoid tobacco exposure, maintain clean air environments, and get annual chest screenings if you smoke."
        )
...
# ══════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        res  = models["thyroid_model"].predict(f)[0]
        prob = models["thyroid_model"].predict_proba(f)[0]
        conf = prob[int(res)]
